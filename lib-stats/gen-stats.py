...
def app_stats():
    ...
    # let's count the number of 3p imports
    num_3p_imports = 0
    libs3p = OrderedDict()
    for cat, l in libs.items():
        libs3p[cat] = remove_stdlib_imports(l)
        num_3p_imports += len(libs3p[cat])
    ...
    write_str(avg_lib_per_app, "Avg number of imports per app")
    avg_3plib_per_app = "%.1f" % (num_3p_imports/num_apps)
    write_str(avg_3plib_per_app, "Avg number of 3p imports per app")
